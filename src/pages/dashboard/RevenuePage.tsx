...
import {
  PageHeader, SectionCard, FilterBar, EmptyState, ErrorState, TableSkeleton,
  Pagination, DateRangeFilter, FallbackBanner,
} from "@/components/shared";
import {
  useRevenueSummary, useRevenueOverTime, useRevenueByStation,
  useRevenueBreakdown, useRevenueTransactions, useStations,
  type DatePeriod,
...
const RevenuePage = () => {
  ...
  const onFilterChange = (cb: () => void) => { cb(); setPage(1); };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <DateRangeFilter
          ...
          onChange={(next) => onFilterChange(() => {
            setPeriod(next.period); setDateFrom(next.date_from); setDateTo(next.date_to);
          })}
        />
      </div>

      {summary.isFallback && <FallbackBanner onRetry={() => { summary.refetch(); overTime.refetch(); byStation.refetch(); breakdown.refetch(); transactions.refetch(); }} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <SectionCard title="Revenue Over Time">
          {overTime.isLoading ? (
            ...
            <EmptyState title="No revenue data" description="No completed transactions in this range." />
          ) : (
        ...
        <SectionCard title="Revenue by Station">
          {byStation.isLoading ? (
            ...
            <EmptyState title="No station revenue" description="No completed transactions in this range." />
          ) : (
            <ResponsiveContainer width="100%" height={260}>
      ...
      <SectionCard title="Transaction Type Breakdown">
        {breakdown.isLoading ? (
          ...
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie
                data={breakdown.data} dataKey="value" nameKey="type"
                cx="50%" cy="50%" outerRadius={90}
                label={({ type, percent }) => `${typeLabel(String(type))} ${(Number(percent) * 100).toFixed(0)}%`}
              >
      ...
      >
        <Select value={stationFilter} onValueChange={(v) => onFilterChange(() => setStationFilter(v))}>
        ...
        <Select value={statusFilter} onValueChange={(v) => onFilterChange(() => setStatusFilter(v))}>
      ...
      {transactions.isLoading ? (
        ...
        <ErrorState title="Couldn't load transactions" message={transactions.error} onRetry={transactions.refetch} />
      ) : (
        <>
              ...
              <tbody>
                {txRows.map((t) => (
                  <tr key={t.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                    <td className="px-4 py-3 text-foreground whitespace-nowrap">{formatDateTime(t.created_at)}</td>
                    <td className="px-4 py-3 text-foreground font-mono text-xs">{t.rental_code ?? "—"}</td>
                    <td className="px-4 py-3 text-foreground">{t.station_name ?? "—"}</td>
                    <td className="px-4 py-3 text-foreground">{t.machine_name ?? "—"}</td>
                    <td className="px-4 py-3 text-foreground capitalize">{typeLabel(t.transaction_type)}</td>
                    <td className="px-4 py-3 text-foreground font-medium">{formatKsh(Number(t.amount))}</td>
                ...
                {txRows.length === 0 && (
                  <tr><td colSpan={9}><EmptyState title="No transactions found" description="Try adjusting your filters or date range." /></td></tr>
                )}
              </tbody>
            </table>
          </div>
