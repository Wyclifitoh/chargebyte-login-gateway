...
exports.list = async (req, res, next) => {
  try {
    ...
    if (station_id) {
      ...
      vals.push(station_id);
    }
    if (q) {
      where.push(
        "(t.ticket_no LIKE ? OR t.subject LIKE ? OR t.customer_phone LIKE ? OR t.customer_name LIKE ?)",
      );
      const like = `%${q}%`;
    ...
    const sql = `
      ...
      LEFT JOIN system_users u1 ON u1.id = t.assigned_to COLLATE utf8mb4_unicode_ci
      LEFT JOIN system_users u2 ON u2.id = t.created_by COLLATE utf8mb4_unicode_ci
      LEFT JOIN cb_stations s    ON s.id = t.station_id COLLATE utf8mb4_unicode_ci
...
exports.assign = async (req, res, next) => {
  try {
    ...
    if (!assigned_to)
      return res
        .status(400)
        .json({ success: false, error: "assigned_to required" });
    await db.query(
...
exports.addComment = async (req, res, next) => {
  try {
    ...
    if (!body || !body.trim())
      return res
        .status(400)
        .json({ success: false, error: "Comment body required" });
