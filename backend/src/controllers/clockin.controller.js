...
exports.clock = async (req, res, next) => {
  try {
    ...
    if (station_id) {
      ...
      if (
        stationRow &&
        stationRow.latitude != null &&
        stationRow.longitude != null
      ) {
        if (latitude == null || longitude == null) {
          return res.status(422).json({
            success: false,
            error:
              "Location required. Enable GPS on your device to clock in at this station.",
            code: "GPS_REQUIRED",
          });
        }
        const defaultRadius = await settings.getInt(
