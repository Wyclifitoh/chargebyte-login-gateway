...
exports.getAllV1 = async (req, res, next) => {
  try {
...
exports.getAll = async (req, res, next) => {
  try {
    ...
    if (station_id) {
      // If filtering by station, join machines and filter by station_id
...
exports.getById = async (req, res, next) => {
  try {
    ...
    if (!rows.length)
      return res
        .status(404)
        .json({ success: false, error: "Rental not found" });
...
exports.cancel = async (req, res, next) => {
  try {
    await db.query(
      "UPDATE rentals SET status = 'cancelled' WHERE id = ? AND status = 'active'",
      [req.params.id],
    );
...
exports.sendSms = async (req, res, next) => {
  try {
    ...
    console.log(
      `[SMS] to=${phone_number} rental=${rental_id || "-"} msg=${message}`,
    );
    ...
    try {
      await db.query(
        ...
        [
          `SMS to ${phone_number}`,
          message,
          JSON.stringify({ phone_number, rental_id, sent_by: req.user?.id }),
        ],
      );
    } catch (e) {
