...
exports.getAll = async (req, res, next) => {
  try {
    ...
    if (search) { conditions.push('(m.name LIKE ? OR m.qr_code LIKE ?)'); values.push(`%${search}%`, `%${search}%`); }
    if (status) { conditions.push('m.status = ?'); values.push(status); }
    if (station_id) { conditions.push('m.station_id = ?'); values.push(station_id); }
    if (req.user.role === 'location_partner') {
      conditions.push('s.host_partner_id = ?'); values.push(req.user.id);
    }
